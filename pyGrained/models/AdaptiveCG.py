...
class AdaptiveCG(CoarseGrainedBase):
    ...
    def write_pdb(self, filename: str):
        ...
        with open(filename, 'w') as f:
            ...
            # This draws the lines/tubes between beads in visualizers.
            # We connect bead `i` to `i+1` ONLY if they are in the same chain.
            # for i in range(len(self.cg_coords) - 1):
            #     # Check if the next bead belongs to the same chain
            #     if self.cg_chains[i] == self.cg_chains[i+1]:
            #         # Check if they are sequential beads (optional sanity check)
            #         curr_id = self.cg_beads_ids[i]
            #         next_id = self.cg_beads_ids[i+1]
                    
            #         if next_id == curr_id + 1:
            #             # PDB Serials are 1-based index, so bead i is serial i+1
            #             atom_serial_1 = i + 1
            #             atom_serial_2 = i + 2
            #             f.write(f"CONECT{atom_serial_1:>5}{atom_serial_2:>5}\n")
